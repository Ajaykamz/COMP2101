...
test -d ~/Pictures/pics && cat <<EOF
Found $(find ~/Pictures/pics -type f|wc -l) files in the Pictures directory.
The Pictures directory uses $(du -sh ~/Pictures/pics|awk '{print $1}') space on the disk.
EOF
